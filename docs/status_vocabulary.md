...
- meaning:
  - `proposed`。该 confirmed row 未来可能被判定为不应继续作为当前 survey 的 active confirmed paper。
- required evidence or artifacts:
  - 仍应保留明确排除理由，例如 duplicate, scientific drift, wrong scope, or superseded selection。
- allowed next statuses:
  - 作为未来设计，可回到 `confirmed`、`ready_for_reading` 或其他更早状态，但前提是先在 plan 中解释并重新批准。
- whether CSV update requires user approval:
  - 是。更重要的是，`scripts/validate_tables.py` 目前并不允许把 `excluded` 写入 `tables/confirmed_papers.csv`；在该 validator 明确更新并获得用户批准之前，agents must not write `excluded` into `tables/confirmed_papers.csv`.
...
这个 vocabulary document 可以描述 `proposed` 或 `reserved` labels，但只有 `scripts/validate_tables.py` 当前允许的 labels 才能写入 CSV `status` 字段。
