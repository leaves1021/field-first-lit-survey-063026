...
| `citekey` | `michaels2016NeuralPopulationDynamics` |
| `year` | `2016` |
| `category` | `theoretical / computational method` 或 `computational/analysis landmark` |
| `field_axis` | `population dynamics and neural manifolds; computational methods and modeling` |
| `species` | `Macaque monkey (PMd/M1 data as example dataset)` |
| `brain_area_or_scale` | `PMd/M1 motor cortex` |
| `task_or_behavior` | `13-direction center-out reaching` |
| `neural_measurement` | `population activity; simulated neural activity; PMd/M1 comparison` |
| `neural_object` | `rotational dynamics; representational tuning; preferred direction; jPCA planes` |
| `computation` | `movement generation; dynamical systems explanation` |
| `causal_status` | `primarily correlational/model-comparison` |
| `key_finding_short` | `CMPT shows that rotational dynamics in PMd/M1 are better explained by dynamical-system logic than by representational tuning alone.` |
| `modeling_opportunity` | `jPCA-style dimensionality reduction; covariance-matched permutation logic; RNN benchmarks for motor cortex dynamics` |
| `identifiers` | `DOI 10.1371/journal.pcbi.1005175; PMID 27814352; PMCID PMC5096671; S2 af1a65fa44773c427235e1f88bf714d64418883f` |
| `uncertainty` | `Real data source details and some panel-to-claim mappings require manual PDF inspection.` |
| `notes` | `Full-text note emphasizes model comparison and methodological caution, not just representational tuning.` |
